			...
			while True:
				...
				if line.strip().upper() == 'NAMELIST':
					...
					if '&' in line:
						...
						line = re.sub(r"('[^']*')| +", lambda x: x.group() if '\'' in x.group() else ' ', line)
						line = re.sub('= ?', '=', line)
						line = line.replace(' ', '(', 1)
						...
						parsing_list = re.findall("(?:\'.*?\'|\S)+", parsing)
						for pi in range(len(parsing_list) - 1, 0, -1):
						...
						line = line[:i] + '' + re.sub(r"('[^']*')|[a-zA-Z]+", lambda x: '' + x.group().upper() + '' if '\'' not in x.group() else x.group().upper(), ' '.join(parsing_list)) + '' + line[j:]
